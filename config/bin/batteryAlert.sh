...
file="$HOME/.config/bin/notifications"
...
if [[ $status -eq 1 && $percentage -eq 100 ]]; then
    if [ "$(grep "full = no" $file)" ]; then 
        export DISPLAY=:0
        notify-send -t 4000 -i $HOME/.config/bin/assets/full.png "Bateria cargada" "$percentage%"
        XDG_RUNTIME_DIR=/run/user/`id -u` aplay -t wav $HOME/.config/bin/assets/notify.wav
    ...
    if [[ $status -eq 0 && $percentage -gt 10 && $percentage -lt 20 && $(grep "20% = no" $file) && $charging -eq 0 ]]; then
        export DISPLAY=:0
        notify-send -t 4000 -i $HOME/.config/bin/assets/low.png "Bateria baja" "$percentage%"
        XDG_RUNTIME_DIR=/run/user/`id -u` aplay -t wav $HOME/.config/bin/assets/notify.wav
        sed 's/20% = no/20% = si/g' $file | sponge $file
    elif [[ $status -eq 0 && $percentage -lt 10 && $(grep "10% = no" $file) &&  $charging -eq 0 ]]; then
        export DISPLAY=:0
        notify-send -t 4000 -i $HOME/.config/bin/assets/critical.png "Conectar cargador" "$percentage%"
        XDG_RUNTIME_DIR=/run/user/`id -u` aplay -t wav $HOME/.config/bin/assets/notify.wav
